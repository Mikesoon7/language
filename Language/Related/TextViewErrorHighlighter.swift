//
//  TextViewErrorHighlighter.swift
//  Learny
//
...
class TextViewErrorHighlighter: NSLayoutManager {
    
    var errorRange: NSRange? = nil
    ...
    override func drawBackground(forGlyphRange glyphsToShow: NSRange, at origin: CGPoint) {
        ...
        guard errorRange != nil else { return }
        
        let visibleGlyphRange = NSRange(location: max(0, glyphsToShow.location ),
                                        length: glyphsToShow.length )
        let glyphRange = self.glyphRange(forCharacterRange: visibleGlyphRange, actualCharacterRange: nil)
        ...
        if NSIntersectionRange(glyphRange, visibleGlyphRange).length > 0 {
            ...
            self.enumerateEnclosingRects(
                ...
                in: container) { rect, _ in
                    ...
                    self.drawBackground(with: .systemRed, rect: adjustedRect)
                }
            
            let visibleGlyphRange = NSRange(location: max(0, glyphsToShow.location ),
                                            length: glyphsToShow.length + 2 )
            
//            if let errorRange = errorRange, NSIntersectionRange(errorRange, visibleGlyphRange).length > 0 {
