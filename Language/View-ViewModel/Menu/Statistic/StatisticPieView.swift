//
//  StatisticPieView.swift
//  Language
//
//  Created by Star Lord on 27/08/2023.
//

import SwiftUI
import Charts

struct DictionaryLogData: Identifiable{
    var id = UUID()
    
    var dictionaryName: String
    var accessCount: Int
}
struct StatisticPieView: View {
    private var data: [DictionaryLogData]
    
    init(data: [DictionaryLogData]){
        self.data = data
    }
    var body: some View {
        ScrollView {
            Chart(data) { dictionary in
                BarMark(x: .value("count", dictionary.accessCount),
                        y: .value("name", dictionary.dictionaryName)
                )
                .annotation(position: .overlay, alignment: .leading, content: {
                    Text("\(dictionary.accessCount)")
                })
