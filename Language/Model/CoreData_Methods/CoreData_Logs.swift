...
protocol LogsManaging{
    func accessLog(for dictionary: DictionariesEntity, secSpent: Int64, cardsChecked: Int64) throws
...
//    }
    ...
    private func createNewLog(for dictionary: DictionariesEntity, at date: Date = Date()) -> DictionariesAccessLog {
        let log = DictionariesAccessLog(context: context)
        log.accessDate = date.timeStripped
        log.accessAmount = 0
        log.accessTime = 0
...
//    }
    func accessLog(for dictionary: DictionariesEntity, secSpent: Int64, cardsChecked: Int64) throws {
        guard let log = fetchLog(for: dictionary) else {
            do {
                let log = createNewLog(for: dictionary)
                log.accessCount += 1
                log.accessAmount += cardsChecked
                log.accessTime += secSpent
                try saveContext()
            } catch {
                throw LogsErrorType.creationFailed(dictionary.language)
            }
